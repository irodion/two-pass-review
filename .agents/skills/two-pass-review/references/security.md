...
names who is harmed and how, in the terms the code itself uses. Do not introduce an acronym or a term
of art the repository does not already contain. When the repository under review carries a `CONTEXT.md`,
use its vocabulary for the domain nouns; when it does not, the code's own names are the vocabulary, and
inventing better ones is not your job.
